...
interface KanbanAddFactDialogProps {
  experimentId?: string;
  initialValues?: { text: string; description: string };
  onDone: (
    text: string,
    description: string,
    tags: string[],
...
export function KanbanAddFactDialog({ experimentId: fixedExpId, initialValues, onDone, onCancel }: KanbanAddFactDialogProps) {
  ...
  const [insightExpFilter,   setInsightExpFilter]   = useState('');
  const [insightsOpen,       setInsightsOpen]       = useState(false);

  const fileRef   = useRef<HTMLInputElement>(null);
  const newTagRef = useRef<HTMLInputElement>(null);

  const canSubmit = text.trim().length > 0 && (fixedExpId != null || expId !== '');

  const allInsights = state.items.filter(i => i.type === 'insight');
  const effectiveExpFilter = fixedExpId ?? insightExpFilter;
  const filteredInsights = allInsights.filter(i => {
    if (effectiveExpFilter && i.experimentId !== effectiveExpFilter) return false;
    if (insightSearch.trim()) return i.text.toLowerCase().includes(insightSearch.toLowerCase());
    return true;
  });

  const toggleInsight = (id: string) =>
    setSelectedInsightIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  ...
  const handleSubmit = () => {
    if (!canSubmit) return;
    onDone(text.trim(), description.trim(), tags, fixedExpId ?? expId, selectedInsightIds, attachment);
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Level level="fact" />
            <h2 style={{ fontSize: 16, fontWeight: 600, color: HF.ink, margin: 0 }}>{isEdit ? 'Edit fact' : 'Add fact'}</h2>
          </div>
          <button
            onClick={onCancel}
            style={{
        ...
        {fixedExp ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <label style={{ fontSize: 12.5, fontWeight: 500, color: HF.ink2 }}>
              Experiment <span style={{ color: '#dc2626' }}>*</span>
            </label>
            <select
              value={expId}
              ...
              style={{
                ...
                border: `1px solid ${expId === '' ? '#dc2626' : HF.border}`,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label style={{ fontSize: 12.5, fontWeight: 500, color: HF.ink2 }}>
            Fact <span style={{ color: '#dc2626' }}>*</span>
          </label>
          <input
            autoFocus
            value={text}
            onChange={e => setText(e.target.value)}
            placeholder="Summarise the observation in one sentence"
        ...
        {/* Connect to insights */}
        <ConnectSection
          title="Supports insight"
          badge={selectedInsightIds.length}
          badgeColor={HF.insight.dot}
          open={insightsOpen}
        ...
        >
          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            ...
            {!fixedExpId && (
              <select
                value={insightExpFilter}
                onChange={e => setInsightExpFilter(e.target.value)}
                style={{
                  border: `1px solid ${HF.border}`, borderRadius: 6,
                  background: HF.bgSubtle, color: insightExpFilter ? HF.ink : HF.ink3,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 200, overflowY: 'auto' }}>
            {filteredInsights.length === 0 ? (
              <span style={{ fontSize: 12.5, color: HF.ink4, padding: '6px 0' }}>No insights match.</span>
            ) : filteredInsights.map(insight => {
              const checked = selectedInsightIds.includes(insight.id);
              const exp = state.experiments.find(e => e.id === insight.experimentId);
              return (
                <button
                  key={insight.id}
                  ...
                  style={{
                    ...
                    background: checked ? HF.insight.bg : 'transparent',
                    border: `1px solid ${checked ? HF.insight.dot : HF.border}`,
                ...
                >
                  <span style={{
                    ...
                    background: checked ? HF.insight.dot : 'transparent',
                    border: `1.5px solid ${checked ? HF.insight.dot : HF.borderStrong}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    {checked && <Icon name="check" size={9} stroke="#fff" />}
                  </span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 12.5, color: checked ? HF.insight.ink : HF.ink, margin: 0, lineHeight: 1.4 }}>
                      {insight.text}
                    </p>
                    {!fixedExpId && exp && (
